	...
	//////////////////////////////////////////////////
	 ...
	 public ResponseEntity<ResetCodeResponse> generateResetCode(String username) {
		    ...
		    try {
		        ...
		        // Generate and send the reset code to the user's email or messages
		        System.out.println("The reset code for " + username + " is " + resetCode);

		        student.setResetCode(resetCode);
		        srepo.save(student);

		        return ResponseEntity.ok(new ResetCodeResponse("Reset code generated successfully. Check your email or messages for the code."));
		    } catch (Exception e) {
		        return ResponseEntity.badRequest().body(new ResetCodeResponse("Failed to generate reset code: " + e.getMessage()));
		    }
		}

	 public String resetPassword(String username, int resetCode, String newPassword, String confirmPassword) {
		    List<StudentEntity> students = srepo.findByUsernameAndIsDeleted(username, false);
